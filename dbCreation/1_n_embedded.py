...
db1 = client["1_n_embedded"]
collection_A1 = db1["A"] 
collection_B1 = db1["B"]

def definingArrayComponents():
    TiposDatos = ["String","Integer","Boolean","Double","Timestamp"]
    dataType = random.choice(TiposDatos)
...
def definingValues():
    TiposDatos = ["String","Integer","Boolean","Double","Timestamp", "Array"]
    dataType = random.choice(TiposDatos)
...
for k in range(0,documents_quantity,range_intervals):
    toInsertA = []
    toInsertB = []
    ...
    for i in range(range_intervals):
        ...
        selected_B_documents = random.sample(toInsertB, N)
        A["B_docs"] = selected_B_documents
        toInsertA.append(A)        
